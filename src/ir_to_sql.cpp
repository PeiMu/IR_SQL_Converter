...
namespace ir_sql_converter {
		...
		switch (op.GetNodeType()) {
				...
				for (size_t idx = 0; idx < proj_op.target_list.size(); idx++) {
					...
					if (table_names.find(target_table_index) == table_names.end()) {
						...
						if (SimplestNodeType::AggregateNode == child_op->GetNodeType()) {
							...
							std::cout << "Do not support yet, child_op node type: " + std::to_string(child_op->GetNodeType()) << std::endl;
							assert(false);
						}
					} else {
						// for the others
			...
			case SimplestNodeType::AggregateNode: {
				...
				for (const auto &agg_fn: agg_op.agg_fns) {
					agg_field.emplace(
						agg_field_key(agg_fn.first->GetTableIndex(), agg_fn.first->GetColumnIndex()),
			...
			case SimplestNodeType::OrderNode: {
				...
				if (SimplestNodeType::ProjectionNode == child_op->GetNodeType()) {
					...
					for (const auto &target: proj_op.target_list) {
						proj_target_list_ids.emplace_back(target->GetTableIndex(), target->GetColumnIndex());
					}
				} else {
					std::cout << "Do not support yet, child_op node type: " + std::to_string(child_op->GetNodeType()) << std::endl;
					assert(false);
...
#ifdef DEBUG
						assert(SimplestNodeType::JoinNode == filter_op.children[0]->GetNodeType());
			...
			case SimplestNodeType::JoinNode: {
				...
				switch (join_type) {
					...
					default:
						std::cout << "Do not support yet, join_type: " + std::to_string(join_type) << std::endl;
						assert(false);
				}
				break;
			}
	...
	std::string IRToSQLConverter::CollectFilter(const std::unique_ptr<SimplestExpr> &qual_expr) {
		std::string ret_str;
		switch (qual_expr->GetNodeType()) {
			case SimplestNodeType::VarConstComparisonNode: {
				...
				switch (var_const_comp.GetSimplestExprType()) {
					...
					default:
						std::cout
							<< "Do not support yet, var_const_comp->type: " + std::to_string(var_const_comp.GetSimplestExprType()) << std::endl;
						assert(false);
				}
			...
			case SimplestNodeType::LogicalExprNode: {
				...
				switch (logical_expr.GetLogicalOp()) {
					...
					default:
						std::cout << "Do not support yet, logical_expr->type: " + std::to_string(logical_expr.GetLogicalOp()) << std::endl;
						assert(false);
						break;
				}
				break;
			...
			case SimplestNodeType::IsNullExprNode: {
				...
				switch (is_null_expr.GetSimplestExprType()) {
					...
					case SimplestExprType::NonNullType:
						...
						return ret_str;
					default:
						std::cout << "Do not support yet, is_null_expr->type: " + std::to_string(is_null_expr.GetSimplestExprType()) << std::endl;
						assert(false);
						break;
				}
				break;
			...
			default:
				std::cout << "Do not support yet, qual_expr->type: " + std::to_string(qual_expr->GetNodeType()) << std::endl;
				assert(false);
		}
		return ret_str;
	}
